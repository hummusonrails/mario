(function () {
	...
	Player.prototype.setAnimation = function () {
		...
		if (this.starTime) {
			...
			this.starTime -= 1;
			if (this.starTime == 0) {
				// Star effect ended: pause star music and resume background music
				if (music.star && !music.star.paused) {
					music.star.pause();
				}
				if (music.overworld) {
	...
	Player.prototype.update = function (dt, vX) {
		...
		if (this.exiting) {
			...
			if (this.pos[0] >= this.targetPos[0]) {
				...
				sendGameplayUpdate({ action: 'levelExit' });
	...
	Player.prototype.star = function (idx) {
		...
		if (music.overworld && !music.overworld.paused) {
			music.overworld.pause();
		}
		if (music.underground && !music.underground.paused) {
			music.underground.pause();
		}
		// Start star music
		if (music.star) {
			music.star.currentTime = 0;
			music.star.play();
		}
	}
